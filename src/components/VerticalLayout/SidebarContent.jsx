...
import { lIST_SIDEBAR_CONTENT } from "../../constants/app.const";
import Cookies from "js-cookie";
import { filteredMenuItems } from "../../utils/app";
import Footer from "./Footer";

const SidebarContent = (props) => {
  const ref = useRef();
  const path = useLocation();
  let listSidebar = Cookies.get("permissionUser")
    ? filteredMenuItems(JSON.parse(Cookies.get("permissionUser")))
  ...
  const activeMenu = useCallback(() => {
    ...
    for (let i = 0; i < items.length; ++i) {
      const match = matchPath({ path: items[i].pathname, end: false }, pathName);
      if (match) {
  ...
  useEffect(() => {
    if (Cookies.get("permissionUser")) {
      listSidebar = filteredMenuItems(
        JSON.parse(Cookies.get("permissionUser"))
      );
    }
  }, [Cookies.get("permissionUser")]);
  // const listSidebar = useMemo(() => {
  //   if(Cookies.get("permissionUser")){
  //     // console.log('a',filteredMenuItems(JSON.parse(Cookies.get("permissionUser"))))
  //     return filteredMenuItems(JSON.parse(Cookies.get("permissionUser")));
  //   }else{
  //     // console.log('b')
  ...
  return (
    <React.Fragment>
      <SimpleBar className="h-100" ref={ref}>
        <div id="sidebar-menu">
          <ul className="metismenu list-unstyled" id="side-menu" onClick={() => {
            ...
            {/* <li>
              <Link to="/#" className="has-arrow">
                ...
                <span>{props.t("Service & Package")}</span>
              </Link>

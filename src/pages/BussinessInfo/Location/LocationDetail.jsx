...
const LocationDetail = (props) => {
  ...
  document.title = `${id ? "" : "Create"} Location | Actiwell System`;
  ...
  const handleSubmitForm = async () => {
    try {
      ...
      const response = await operatorService.createLocation(formData);
      if (response.success) {
  ...
  const handleFileChange = (e) => {
    ...
    if (files.length) {
      files.forEach((file) => {
        ...
        validation.setFieldValue(
          "images",
          validation.values.images.concat(file)
        );
  ...
  const handleCheckboxChange = (event, itemId) => {
    const updatedCheckedAmenities = event.target.checked
      ? [...validation.values.amenities_ids, itemId]
      : validation.values.amenities_ids.filter((id) => id !== itemId);
    validation.setFieldValue("amenities_ids", updatedCheckedAmenities);
  };
  const handleGetLocationDetail = async (id) => {
    try {
      ...
      if (response.success) {
        const resData = {
          ...response.data,
          amenities_ids: response.data.amenities,
        };
        ...
        <div className="tabs-container">
          ...
          <div className="page-container">
            ...
            >
              ...
              <div className="tabs-header" style={{ position: 'sticky', top: 0, zIndex: 10 }}>
              ...
              <Card className="mb-4">
                <CardBody>
                  ...
                  <LayoutImage>
                    ...
                    <div>
                      <UploadButton
                        as="label"
                        htmlFor={
                          props.type === "create" ? "images" : "images_upload"
                        }
                        className={
                  ...
                  {validation.errors.images && (
                    <InvalidFeedback>
                      <p>
                        {fileName} {validation.errors.images}
                      </p>
                    </InvalidFeedback>
                  )}
                </CardBody>
                          ...
                          <Col className="my-2" key={item.id}>
                            <div className=" d-flex justify-content-between">
                              ...
                              <input
                                ...
                                //   item.id
                                // )}
                                defaultChecked={validation.values.amenities_ids.includes(
                                  item.id
                                )}
                                disabled={false}
                                onChange={(e) =>

...
const ProductCategory = (props) => {
  ...
  const createCategory = useMemo(() => {
    return permissionUser.includes("product_category:create");
  }, [permissionUser]);
  const updateStatus = useMemo(() => {
    return permissionUser.includes("product_category:update_status");
  }, [permissionUser]);
  const viewDetail = useMemo(() => {
    return permissionUser.includes("product_category:view_detail");
  }, [permissionUser]);
  const deleteProduct = useMemo(() => {
    return permissionUser.includes("product_category:delete");
  }, [permissionUser]);
  ...
  const handleUpdateStatus = async (id, active) => {
    try {
      const res = await categoryService.updateStatusCategory(
        id,
        active === 1 ? 0 : 1
      );
      if (res.success) {
  ...
  const handleGetProductList = async () => {
    try {
      ...
      setSelectedProduct([]);
      const res = await categoryService.getListCategory(payload);
  ...
  const handleDeleteMulti = async () => {
    try {
      const res = await categoryService.deleteMultiCategories(selectedProduct);
      if (res.success) {
        handleGetProductList();
      }
    } catch (e) {
  ...
  const titleModal = useMemo(() => {
    return deleteId
      ? i18n.t("popup_del_this_category")
      : i18n.t("popup_del_selected_categories");
  }, [deleteId]);
    ...
    <div className="d-flex flex-column flex-1">
      <div className="filter-container">
        <div className="filter-header">
          <div className="filter-title-group" onClick={() => setFilterOpen(!filterOpen)}>
            ...
            <h5 className="filter-title">{i18n.t("product_category_list")}</h5>
          </div>
          <button
      ...
      {products.length > 0 ? (
              ...
              <tbody>
                {products.map((item, index) => (
                  <tr key={index} onClick={() => handleRedirect(item)}>
                    ...
                    <td>{item.product_category_number}</td>
                    <td>{item.name}</td>
                    <td>
                      <div
                        style={{
        ...
        <div className="d-flex justify-content-center d-flex align-items-center h-100 mt-4">
          <div>{i18n.t("no_category_exist")}</div>
        </div>
      )}
